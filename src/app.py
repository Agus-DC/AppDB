...
def profile(name):
    ...
    if user1 and user1.get_islogin() and (user1.get_username() == name):
      ...
      return render_template('home.html', form=form, info = info, value = name);
...
def upload():
    ...
    # Si el usuario no esta logeado no puede acceder a la opcion upload
    #if request.method == 'GET':
    #  user1 = usuario.get_user(name)
    #  if (user1==None or ((user1.get_islogin()==False) and (user1.get_username() == name))):
